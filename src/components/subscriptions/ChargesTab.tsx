...
export function ChargesTab({ companyId }: ChargesTabProps) {
  ...
  const handleMarkAsPaid = async (charge: any) => {
    ...
    try {
      ...
      // 3. Register commission if applicable (Phase 1 simplicity: using the finance display logic)
      // Note: If there's a dedicated 'commissions' table, we should insert there.
      // Looking at FinanceCommissions.tsx, it seems to aggregate from appointments.
      // For Phase 1, the revenue record linked to the professional should suffice for basic tracking.
