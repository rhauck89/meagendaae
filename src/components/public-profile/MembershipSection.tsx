  ...
  return (
    <section className="w-full px-2 animate-fade-in mb-6">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-lg p-0 overflow-hidden border-none bg-slate-900 text-white rounded-3xl">
          <DialogHeader className="p-6 pb-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-amber-500 flex items-center justify-center text-slate-900 shadow-lg">
          ...
          <DialogBody className="p-6 pt-2 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
            <div className="space-y-4">
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-bold text-white tracking-tight">
                  R$ {Number(selectedPlan?.price_monthly || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </span>
                <span className="text-xs font-semibold text-white/30 uppercase tracking-widest">/mês</span>
              </div>

              {selectedPlan?.price_yearly && Number(selectedPlan.price_yearly) > 0 && (
                <div className="px-3 py-2 rounded-xl bg-amber-500/10 border border-amber-500/20">
                  <span className="text-xs font-bold text-amber-500 uppercase tracking-wider">
                    Opção Anual: R$ {Number(selectedPlan.price_yearly).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5">
                ...
                <p className="text-sm font-semibold text-white">
                  {selectedPlan?.type === 'unlimited' ? 'Ilimitado' : `${selectedPlan?.usage_limit} por mês`}
                </p>
              </div>
              <div className="p-3 rounded-2xl bg-white/[0.03] border border-white/5">
                <div className="flex items-center gap-2 mb-1">
                  <Calendar className="w-3.5 h-3.5 text-amber-500" />
                  <span className="text-[10px] font-bold text-white/40 uppercase tracking-wider">Serviços</span>
                </div>
                <p className="text-sm font-semibold text-white">
                  {selectedPlan?.included_services?.length || 0} inclusos
                </p>
              </div>
            </div>

            <div className="space-y-3">
              <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-wider flex items-center gap-2">
                <Info className="w-3.5 h-3.5" />
                Regras e Observações
              </h4>
              <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/5">
                <p className="text-sm text-white/80 whitespace-pre-wrap leading-relaxed font-mono text-[11px]">
                  {selectedPlan?.observations || 'Nenhuma regra adicional informada.'}
                </p>
              </div>
            </div>
          </DialogBody>
          ...
          <div className="p-6 pt-2 bg-slate-900">
            <Button 
              ...
              className="w-full h-12 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-900 font-bold gap-2 shadow-lg shadow-amber-500/20"
            >
            ...
            <Button 
              ...
              className="w-full mt-2 text-white/40 hover:text-white hover:bg-white/5 text-xs"
            >
              Fechar
            </Button>
          </div>
