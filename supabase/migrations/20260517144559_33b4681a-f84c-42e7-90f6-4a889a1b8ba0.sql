...
BEGIN
  ...
  IF v_profile_id IS NOT NULL THEN
    -- Check if collaborator in the active company
    SELECT EXISTS (SELECT 1 FROM public.collaborators col WHERE col.profile_id = v_profile_id AND (col.company_id = v_company_id OR v_company_id IS NULL) AND col.active = true) INTO v_is_collaborator;
    ...
    -- Apply presets if permissions are empty
